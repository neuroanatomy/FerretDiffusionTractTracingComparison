    ...
    for y in range(conMat.shape[1]):
        for x in range(conMat.shape[0]):
            if conMat[x, y] == 0:
                ...
                conMat_norm[x, y] = conMat[x, y]/(np.sum(conMat[x, :]))

    if not os.path.exists(saveDir):
        os.makedirs(saveDir)
    np.savetxt(saveDir+fileName+'_normFLN'+extension, conMat_norm, delimiter=' ')
...
for f in files:
    ...
    fileName = fileName + '_dzero'
    normalize(saveDir, fileName, extension, saveDir)
    fileName = fileName + '_normFLN'
    symeterize(saveDir, fileName, extension, saveDir)

    fileName = f
    plusOne(path, fileName, extension, saveDir)
    fileName = fileName + '_1plus'
    diagZero(saveDir, fileName, extension, saveDir)
    fileName = fileName + '_dzero'
    normalize(saveDir, fileName, extension, saveDir)
    fileName = fileName + '_normFLN'
    symeterize(saveDir, fileName, extension, saveDir)
